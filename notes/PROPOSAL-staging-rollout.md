# Staging rollout proposal — git loader rehaul + concurrent content_add + size-based dispatch
...
> **Branch-name note:** Branch names below correspond to the current MR stack — `mr/1-gix-engine` → `mr/2-size-classed-queues` → `mr/3-dulwich-fallback` → `mr/4-helm-overlay` (in swh-loader-git, open as drafts `!217`–`!220`); the concurrent `content_add` MR stack on swh-storage is detailed in `notes/git-loader-rehaul/PLAN-concurrent-content-add.md`. Lane 2 (scheduler-side `feat/size-based-dispatch-v1`) and Lane 3 (lister-side `feat/github-size-metadata`) remain deferred.
...
(concurrent `content_add` on swh-storage; MR sequence ready to push per `notes/git-loader-rehaul/PLAN-concurrent-content-add.md`), and built the full
...
is addressed by concurrent content_add's write-path concurrency with an expected 16–65×
speedup.  All five branches
are committed and test-covered; we propose a staged rollout where
zero-config pipeline improvements land first, concurrent content_add lands behind a
...
|---|---|---|---|---|
| swh-loader-git | `mr/1-gix-engine` through `mr/4-helm-overlay` (`!217`–`!220`, drafts) | Full gitoxide rehaul (Phases 1–6C, A0, A, B) + dulwich-fallback + size-classed queues + helm overlay | 5+1+3+1 commits across the 4 stacked MRs; see `notes/git-loader-rehaul/HANDOFF.md` §2 for per-MR detail | med |
| swh-storage | concurrent `content_add` MR stack (ready to push) | concurrent Cassandra content writes (write path only — Nicolas's Sept 2025 commits already batched the read path) | per `PLAN-concurrent-content-add.md`: init refactor + concurrent path + bench + reconciler | med |
| swh-scheduler | `feat/size-based-dispatch-v1` (Lane 2, deferred) | Schema + size-aware `grab_next_visits` | `fec39d0` mypy prep; `642f2a6` add `pack_size_kb`+`commit_count` to `listed_origins` (+ `sql/upgrades/39.sql`); `28747d5` add `size_class` filter | low |
| swh-lister | `feat/github-size-metadata` (Lane 3, deferred) | Opt-in GitHub per-repo size collection | `ed711c4` mypy prep; `8b35f1e` github lister: optional `collect_metadata` | low |
...
- **Concurrent `content_add`** (MR sequence specified in `notes/git-loader-rehaul/PLAN-concurrent-content-add.md`, ready to push): adds
  ...
  checks); concurrent content_add is the next-stage optimization, targeting the **write**
...
### 3.3 Projected concurrent content_add impact
...
| Metric | Sequential (today) | Concurrent  | Ratio |
|---|---|---|---|
...
| 4 | concurrent content_add canary | storage config `content_add_algo: "concurrent"` on one writer | scrubber finds no new partial-index states after 7 days; Cassandra flush wall-time ↓ | flip flag back to `"sequential"` |
...
mitigation discussed in the concurrent `content_add` architectural issue
(`notes/git-loader-rehaul/ISSUE-concurrent-content-add.md`) for the new
...
- **Production Cassandra write latency.** The model measures
  loader-only cost with in-memory storage. concurrent content_add pending.
...
- **Phase C (`directory_add_raw`)**: ~15% additional gain on the kernel
  ...
  change. Defer until concurrent content_add lands and we can remeasure.
...
3. **Scrubber cadence after concurrent content_add rollout**: ad-hoc after each large
...
8. **Phase C / from_disk / Mercurial / Bazaar** — would the team prefer
   these bundled into a single follow-up release after concurrent content_add lands,
...
`notes/git-loader-rehaul/ISSUE-concurrent-content-add.md` (architectural issue body),
`notes/git-loader-rehaul/PLAN-concurrent-content-add.md` (MR execution sequence),
`notes/git-loader-rehaul/EXECUTIVE-SUMMARY.md` (entry-point summary),
