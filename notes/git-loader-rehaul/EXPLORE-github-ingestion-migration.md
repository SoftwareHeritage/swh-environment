...
**Scenario B is the awkward middle.** It only makes sense if AdAstra is deferred *and* github-ingestion is going to keep running at scale for many months. In most plausible futures (either AdAstra wins, or REC-L4 + gix-loader rehaul absorbs the throughput need), Scenario B is overkill.
...
- **REC-L4** (`ISSUE-rec-l4-architecture.md`, `PLAN-rec-l4-execution.md`): orthogonal. github-ingestion writes to ORC, not Cassandra; REC-L4 is a Cassandra-side optimization. No interaction.
