# REC-L4 Safe Landing — Issue + MR Sequence

## Context

REC-L4 changes `CassandraStorage._content_add` from 5 sequential CQL round-trips per content (4 indexes + main) to one concurrent batch via `execute_concurrent`. Throughput target: ~5–10× on the content-add hot path.
...
Body content is in `notes/git-loader-rehaul/ISSUE-rec-l4-architecture.md` — ready to copy/paste verbatim.
...
- **Reviewers**: Nicolas Dandrimont (primary, REC-L4 prior-art), David Douard / Valentin Lorentz (secondary).
...
- `swh-storage/swh/storage/cassandra/storage.py` — REC-L4 surface; refactor target for MR1; dispatch for MR2.
