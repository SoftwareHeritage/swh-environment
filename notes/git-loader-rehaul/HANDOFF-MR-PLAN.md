...
- **Five deployment stages**, each shippable on its own. **Stage 0** is a 91-line production bug fix that's already-deployable today. **Stage 1** is the gix engine as a drop-in replacement (no fallback, no knobs). **Stages 2–4** add the dulwich fallback, size-classed queues, and REC-L4 in turn. Each stage commits the team to more code but unlocks more capability.
- **Two reviewers carry most of the load**: a senior engineer (estimate 1–1.5 days for the full rehaul) and an operator (estimate ~½ day for the deploy-surface). REC-L4 is independent (~2 hours).
- **Yes, history can be rewritten cleanly.** The current 42-commit graph compresses to ~13 commits across 5 MRs without information loss; the recipe is in §5.
...
`swh-storage/feat/content-add-concurrent` vs master (REC-L4):
...
### Stage 4 — REC-L4 concurrent `content_add` (write-path optimization)
...
### 3.5 swh-storage REC-L4 — 239 LOC
...
### 4.3 Storage owner (Nicolas Dandrimont–profile reader, REC-L4): ~2 hours
...
## 5. MR sequence and history rewrite — landed 2026-05-06

**Status: complete locally.** The history rewrite below has been executed; the resulting branches `mr/1-gix-engine`, `mr/2-size-classed-queues`, `mr/3-dulwich-fallback`, `mr/4-helm-overlay` (swh-loader-git) and `mr/5-rec-l4-content-add` (swh-storage) live in the local repos. Three departures from the originally-proposed plan were found necessary; documented inline below.



### 5.1 Why rewrite

The current 42-commit `feat/gix-dispatch-integrated` was built incrementally during a multi-week rehaul. The commits are honest history (Phase 1 → 6C, then optimisations, then typed exceptions, then dulwich-fallback bolted on top), but they don't *navigate well as MRs*: phases are tiny, names are internal-jargon, and an early-phase commit may be obsoleted by a later phase. A reviewer needs to read all 42 to understand what shipped.

A clean rewrite preserves the engineering content but presents it in **5 logical MRs of ~3 commits each** — a reviewer reads commit-by-commit and the diff makes sense at every step.

### 5.2 Final MR sequence (as built)

> **Departure 1**: MR #0 (the production bug fix as a stand-alone MR) was DROPPED. The (2, Directory) crash lives on the gix `_convert_object` path, so it can only land alongside the gix code. Its fix is folded into MR #1's "wire gix into loader" commit.

> **Departure 2**: MR #2 ↔ MR #3 swap. The dulwich-fallback wiring references the size-class tier mappings (`_DULWICH_FALLBACK_TASK[size]`), so size-classed queues must land first. Final order: gix engine → size-classed queues → dulwich-fallback → helm overlay.

> **Departure 3**: REC-L4 split into 2 commits, not 3. The planned commit 3 (concurrency knob + parametrised tests, R4+R8) is genuinely new work and stays on the punch-list.

#### MR #1 — `mr/1-gix-engine` (5 commits, ~7.7k LOC) — landed
...
#### MR #2 — `mr/2-size-classed-queues` (1 commit, ~620 LOC) — landed
...
#### MR #3 — `mr/3-dulwich-fallback` (3 commits, ~1.6k LOC) — landed
...
#### MR #4 — `mr/4-helm-overlay` (1 commit, ~360 LOC) — landed
...
#### MR #5 — `mr/5-rec-l4-content-add` (2 commits, ~240 LOC, separate repo) — landed
On `swh-storage`. Splits the single `4e51c838` commit into two:

1. **`b4d49a40` — cassandra: class-level `_content_add_algo` default for subclass compat**. 6 LOC. (Cherry-pickable onto master alone if a regression appears before the rest of REC-L4 lands.)
2. **`9bf92395` — cassandra: add concurrent content_add path (REC-L4)** (cql.py + storage.py + metrics.py + tests). 233 LOC.

The planned MR #5 commit 3 (R4+R8: `content_add_concurrency` knob + parametrised tests across sequential/concurrent) is genuinely new work and remains on the punch-list (HANDOFF.md §7 #4).

### 5.3 The rebase recipe (operational)

For each MR, the operator does:

```
# MR #1 example — gix engine
git checkout -b mr/1-gix-engine master
git merge --squash feat/gix-typed-exceptions     # collapse 31 commits to staged changes
# then `git reset` selectively to break into the 5 logical commits above:
git reset HEAD                                    # un-stage everything
git add gix-lib/                                  # commit #1 surface
git commit -m "gix-lib: add gix engine (fetch, pack inflate, parsers, traverse)"
git add gix-py/Cargo.toml gix-py/src/lib.rs swh/loader/git/_gix.py swh/loader/git/_gix.pyi
git commit -m "gix-py: add PyO3 bindings for the gix engine"
# ... and so on per the §5.2 commit list
```

Add the file:// fix (`ef34d76`) and TLS backend (`0c813c6`) on top of MR #1 commit #1 / #4 as appropriate — they're regression-fixes for the engine itself, not separate commits.

### 5.4 What gets lost in the rewrite

The phase-by-phase incremental development (Phase 4B Approach A → Phase 4C → Phase 6C) becomes opaque. **This is fine for an MR reader** but means future debugging of a regression introduced *between phases* loses the signal. Mitigation: keep the original `feat/gix-typed-exceptions` branch as a tag (`gix-rehaul-v0-history`) for forensic reference if needed.

### 5.5 What stays

- **All commit content** is preserved (just regrouped).
- **All tests** are preserved.
- **All commit messages** are rewritten as squash messages — the MR description carries the navigability burden, not individual commit subject lines.
- **The session's three regression fixes** (TLS, file://, InMemoryStorage) land inside MRs #1 and #5, not as separate commits — they're integral to the MRs they live in.
...
| Risk | Mitigation |
|---|---|
| Rebase loses a commit | Tag `gix-rehaul-v0-history` before starting. Diff `gix-rehaul-v0-history` vs the rebased branch tip — should be empty for any MR. |
| Reviewer asks for a refactor that requires re-rebasing | Each MR is a single squash branch; refactoring per reviewer feedback is just amending the MR's branch tip, not the upstream rebase. |
...
| REC-L4 `content_add_concurrency` default too high | Default 50 (vs cassandra-driver's 100); knob-tunable per-deploy. Documented in HANDOFF.md §7 #4. |
