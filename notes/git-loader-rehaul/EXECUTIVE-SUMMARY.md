# Executive summary — Git loader rehaul + REC-L4 proposal
...
A **sibling proposal (REC-L4)** covers the storage side: a concurrent code path for `CassandraStorage._content_add` that fires the 5N CQL round-trips per content batch through `cassandra.concurrent.execute_concurrent` instead of sequentially. Independent reviewer track in swh-storage; ships behind a config flag, default sequential, no behavior change unless enabled. See `notes/git-loader-rehaul/ISSUE-rec-l4-architecture.md` for the architectural-issue body and `notes/git-loader-rehaul/PLAN-rec-l4-execution.md` for the 8-MR execution sequence.
...
- **REC-L4 concurrent `content_add` enable** (Phase 3 gate): the optimization is opt-in via storage config (`content_add_algo: concurrent`); enabling requires the journal-driven reconciler to be running with low lag in production first. See `PLAN-rec-l4-execution.md`.

---

## 3. Reviewable surface

5 stacked MRs in `swh-loader-git` + 1 independent MR in `swh-storage`. All branches are local-only as of this writing; push order is in `HANDOFF.md` §7.
...
REC-L4 sibling proposal adds **239 LOC** in swh-storage (4 files in `swh/storage/cassandra/` + `metrics.py` + tests). Independent review track.
...
| Storage owner (REC-L4 — `mr/5-rec-l4-content-add`, separate repo) | ~2 | Already familiar with prior batched-read work |
...
swh-storage / master
  ↓
mr/5-rec-l4-content-add         (2 commits — class-level default, concurrent content_add path)
                                — independent of the gix stack; separate reviewer track
```

Pre-rewrite forensic tags retained: `gix-rehaul-v0-history`, `fix-branch-v0-history`, `rec-l4-v0-history` (see `HANDOFF.md` §2).
...
|---|---|---|
| Decision-makers / leadership | this summary + `notes/presentations/SESSION_B_TEAM.md` (slide deck) | `HANDOFF.md` §0 for current state |
...
| Engineers wanting hands-on | `notes/presentations/SESSION_A_ENGINEERS.md` + `notes/git-loader-rehaul/test-rig/Dockerfile` | run the test rig, check out the branches |
| REC-L4 reviewers | `ISSUE-rec-l4-architecture.md` (issue body, ready for GitLab) + `PLAN-rec-l4-execution.md` (8-MR sequence) | `HANDOFF.md` §7 for open decisions |
...
From `HANDOFF.md` §7 (gix proposal) + `ISSUE-rec-l4-architecture.md` §7 (REC-L4):
...
2. **MR ordering / push order** (loader maintainers) — recommendation in `HANDOFF.md` §7 #5 is `mr/1` → `mr/2` → `mr/3` → `mr/4` for the gix stack, then `mr/5` independently. Confirm before pushing.
3. **REC-L4 reconciler location** — recommend sub-package `swh.storage.reconciler` (alternatives in `PLAN-rec-l4-execution.md` §3). Storage owners decide.
4. **REC-L4 concurrency knob default** — recommend `content_add_concurrency: 50` (vs cassandra-driver default 100). Conservative for first prod deploy; ops + storage owners decide.
5. **REC-L4 R4 + R8 before-MR items** — the `mr/5-rec-l4-content-add` branch currently lacks the concurrency knob (R4, ~10 LOC) and parametrised tests across `sequential` / `concurrent` (R8). Both should land before the MR opens.
6. **MR3 bench harness location** — recommend `swh-storage/swh/storage/tests/bench/content_add.py` (in-tree). Storage owners decide.
...
## 9. REC-L4 sibling proposal — pointer

The Cassandra REC-L4 work ships independently (different repo, different reviewer track, opt-in via storage config). Read `notes/git-loader-rehaul/ISSUE-rec-l4-architecture.md` for the architectural-issue body (copy/paste-ready into a GitLab issue in `swh/devel/swh-storage`) and `notes/git-loader-rehaul/PLAN-rec-l4-execution.md` for the 8-MR sequence (refactor → REC-L4 code → bench → reconciler → 4 deploy gates → optional miss-tolerant proxy).
...
2. **`notes/presentations/SESSION_B_TEAM.md`** — visual narrative of the rehaul story (slide deck, 15-20 min).
3. **`HANDOFF.md` §0 (status) + §2 (branches)** — what's in each MR (5 min).
...
7. **For REC-L4 specifically**: `ISSUE-rec-l4-architecture.md` → `PLAN-rec-l4-execution.md` → the `mr/5-rec-l4-content-add` diff.
